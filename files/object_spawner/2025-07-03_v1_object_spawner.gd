...
var sapling_scene: PackedScene = preload("res://scenes/world/objects/harvestables/sapling.tscn")
#endregion
...
func spawn_harvestable_tree(position: Vector2, island: Island, tilemap_location: Vector2i, tree: SaplingItem) -> bool:
	if not multiplayer.is_server():
		_request_spawn_harvestable_tree.rpc_id(1, position, island.get_path(), tilemap_location, tree.id)
		return false

	var harvestable_tree: Sapling = can_spawn_hittable_object(sapling_scene, position)
