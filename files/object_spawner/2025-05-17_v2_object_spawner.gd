...
func attempt_spawn_hittable_object(object_scene: PackedScene, position: Vector2i, can_spawn_on_bridge: bool = true, existing_object_id: String = "") -> Node:
	var object: HittableObject = can_spawn_hittable_object(object_scene, position, can_spawn_on_bridge)
	
	if not object:
		print("Failed to spawn hittable object")
		return null
	
	var untargetable_tiles: Array[Vector2i] = []
	for x in range(object.hitbox_size.x):
		for y in range(object.hitbox_size.y):
			if object.not_targetable_tiles.has(Vector2i(x, y)):
	...
	object_instance.occupied_tiles = object.occupied_tiles.duplicate()
	print(object_instance.occupied_tiles)
	...
	ObjectDataManager.all_objects[existing_object_id] = object_instance.save()
