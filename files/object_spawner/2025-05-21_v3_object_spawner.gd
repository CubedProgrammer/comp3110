...
func spawn_harvestable_tree(position: Vector2i, island: Island, tilemap_location: Vector2i, tree: SaplingItem) -> Node:
	if not multiplayer.is_server():
	...
	#print(harvestable_tree.save())
	var object_id = ObjectDataManager.register_object(harvestable_tree.save())
	return ObjectDataManager.active_objects.get(object_id, harvestable_tree)
