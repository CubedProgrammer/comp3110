...
@onready var energy_bar: TextureProgressBar = $PlayerUI/HUD/EnergyBar
@onready var health_bar: TextureProgressBar = $PlayerUI/HUD/HealthBar
@onready var direction_point: Node2D = $DirectionPoint
@onready var light: PointLight2D = $Light
@onready var in_game_menu: PanelContainer = $PlayerUI/InGameMenu
...
func _ready() -> void:
	...
	player_data.health_changed.connect(func(value): health_bar.value = value)
	player_data.energy_changed.connect(func(value): energy_bar.value = value)
	
	if not multiplayer.is_server():
		request_load_player_data.rpc_id(1, config.get_value("player", "player_uid"))
	else:
		request_load_player_data(config.get_value("player", "player_uid"))
...
func _notification(what):
	#TODO if you are the host you need to save your data and quit only after the world has been saved
	if what == NOTIFICATION_WM_CLOSE_REQUEST and not multiplayer.is_server():
		var config = ConfigFile.new()
		config.load("user://machine_info.cfg")
...
@rpc("any_peer", "call_local", "reliable")
func save(uid, player_data, quit: bool = false) -> void:
	if not multiplayer.is_server():
		return
	...
	save_complete.rpc_id(multiplayer.get_remote_sender_id(), quit)


@rpc("any_peer", "call_remote", "reliable")
func save_complete(quit: bool) -> void:
	if quit:
		get_tree().quit()
	else:
		#TODO make this dc u from the lobby/server and put u back to the main menu note: you may alr get dced somewhere in the steam network manager
		pass
