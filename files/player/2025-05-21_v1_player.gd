...
func load_player_data(save_data):
	...
	($AnimatedSprite2D as AnimatedSprite2D).sprite_frames = load(save_data["sprite_frames"]) as SpriteFrames
	...
	while temp_hotbar.size() < inventory.hotbar.max_inventory_length:
		temp_hotbar.append(InventoryItem.new("filler", -1))
...
func load_other_info(info: Dictionary) -> void:
	...
	($AnimatedSprite2D as AnimatedSprite2D).sprite_frames = load(info["sprite_frames"]) as SpriteFrames
	visible = true
	state_manager.input_disabled = true
