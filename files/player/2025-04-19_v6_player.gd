...
func _process(delta: float) -> void:
	...
	if not %StateChart.get_expression_property("ui"):
...
func recalculate_closest_object():
	...
	for object in interactable_objects_in_range:
		if position.distance_to(object.position) < position.distance_to(closest_object.position) if closest_object else 10000000000:
