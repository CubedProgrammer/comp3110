...
func can_tile_be_planted(interacted_tile: Vector2i, item: PlantableItem) -> bool:
	var tile = get_island_tile_from_vector(interacted_tile)
	return tile and tile.can_tile_be_planted(item is SaplingItem)

func can_tile_be_harvested(interacted_tile: Vector2i) -> bool:
	var tile = get_island_tile_from_vector(interacted_tile)
	return tile and tile.can_tile_be_harvested()
	
func can_tile_be_placed_on(interacted_tile: Vector2i) -> bool:
	var tile = get_island_tile_from_vector(interacted_tile)
	return tile and tile.can_tile_be_placed_on()
...
func spawn_harvestable_object(tile_position: Vector2i, tree: bool = false) -> void:
	if not is_multiplayer_authority():
		return
	...
	if not tree:
		...
		object_spawner.spawn_harvestable_tree(
			((layout_position * 18) + tile_position) * 64,
			self,
			tile_position
		)

func can_spawn_harvestable_object(tile_position: Vector2i) -> bool:
	var obj = object_spawner.can_spawn_hittable_object(
		preload("res://scenes/world/objects/harvestable_tree_object.tscn"),
		((layout_position * 18) + tile_position) * 64,
		false,
		self,
		1,
