...
var items_for_machine_array: Array[InventoryItem]
...
#some plants also dont break on harvest
func get_harvest_items(tile: Tile , is_machine_harvest: bool = false):
	items_for_machine_array.clear()
	if(tile.plant_on_tile == null):
		return
	...
	if(should_spawn_item_based_on_percent(tile.plant_on_tile.percent_chance_to_drop_self_on_break)):
		...
		if(is_machine_harvest == false):
			...
			return
		items_for_machine_array.append(InventoryItem.new(tile.plant_on_tile.id , seed_drop_amount))

	for item_dict_entry in tile.plant_on_tile.drop_item_id_array:
		if should_spawn_item_based_on_percent(item_dict_entry.percent_to_drop):
			var item_drop_amount = DropableItem.get_amount_based_on_average(
				item_dict_entry.average_drop,
				item_dict_entry.min_drop,
				item_dict_entry.max_drop
			)
			if(is_machine_harvest == false):
				object_spawner.spawn_dropable_item(
					TileManager.get_world_position(layout_position , tile.tile_position),
					item_dict_entry.item_to_drop,
					item_drop_amount, 
					true
				)
			items_for_machine_array.append(InventoryItem.new(item_dict_entry.item_to_drop , item_drop_amount))

func get_machine_item_array() -> Array:
	return items_for_machine_array
