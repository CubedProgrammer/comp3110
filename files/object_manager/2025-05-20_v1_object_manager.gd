...
class WorldInfo:
	...
	func _init(id: String, new_world_name: String, new_icon: WorldSelectPlanet.Planet, tp: int, cs: int) -> void:
		world_id = id
...
func _on_quit() -> void:
	...
	if not multiplayer.is_server() or not ObjectManager.world:
		return
