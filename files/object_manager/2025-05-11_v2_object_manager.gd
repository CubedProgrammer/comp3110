...
func _on_quit() -> void:
	...
	DirAccess.make_dir_recursive_absolute("user://worlds/" + current_world_uid + "/")
	var save_file = FileAccess.open(
		"user://worlds/" + current_world_uid + "/savegame.save", FileAccess.WRITE
	)
...
func load_data():
	...
	if not FileAccess.file_exists("user://worlds/" + current_world_uid + "/savegame.save"):
		return  # We don't have a save to load.

	var save_file = FileAccess.open(
		"user://worlds/" + current_world_uid + "/savegame.save", FileAccess.READ
	)
