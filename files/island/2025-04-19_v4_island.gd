...
func reset_tilled_tile_timer(vector_for_tile_to_till: Vector2i):
	var tile: Tile = get_island_tile_from_vector(vector_for_tile_to_till)
	if(!does_island_tile_state_have_tile(tile.tile_position)):
		return
	if tile.tile_state == TileManager.TileState.Tilled:
		tile.dirt_timer = biome.dirt_timeout_value
