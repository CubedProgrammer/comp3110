...
@export var count_of_resources_spawned_on_first_unlock: int = 10
...
@export_group("Island Worth")
@export var island_worth: int
@export var island_worth_generation_offset: int # Used int the tool when generating the value above
...
@onready var floors_layer: TileMapLayer = $Tilemaps/FloorsLayer
@onready var addons_layer: TileMapLayer = $Tilemaps/AddonsLayer
@onready var walls_layer: TileMapLayer = $Tilemaps/WallsLayer
@onready var tilemaps: Node2D = $Tilemaps
@onready var water_placeholder: TileMapLayer = $WaterPlaceholder
@onready var island_resource_manager: IslandResourceManager = $IslandResourceManager
...
signal plant_harvested(coords: Vector2i)
...
func _ready() -> void:
	...
	stitching_manager.setup_water_placeholder()
	
	if cost == 0 and is_multiplayer_authority():
		if not islands_layout.finished_initial_load:
...
func _process(delta: float) -> void:
	if not is_node_ready() or not multiplayer.is_server():
		return
...
func try_unlock(play_anim: bool = false) -> void:
	if not is_multiplayer_authority():
		return

	_unlock.rpc(play_anim)


@rpc("authority", "call_local", "reliable")
func _unlock(play_anim: bool = false) -> void:
	if unlocked:
		return
	
	islands_layout.island_changed.emit(layout_position)
	QuestsManager.island_unlocked.emit(layout_position)
	water_layer.visible = false
	y_sort_enabled = false
	tilemaps.z_index = 10
	
	if play_anim:
		_start_unlock_with_animation()
	else:
		unlocked = true
		tilemaps.visible = true
		_unlock_sequence(play_anim)




func _start_unlock_with_animation():
	var moar_npc = QuestsManager.npcs[NPC.NAME.Moar] as MOAR
	moar_npc.island_unlocking.emit(layout_position)
	
	moar_npc.camera_reached_island.connect(_play_island_appear_animation, CONNECT_ONE_SHOT)


func _play_island_appear_animation():
	var original_scale = tilemaps.scale
	
	tilemaps.scale = Vector2.ZERO
	tilemaps.visible = true
	...
	unlock_tween.tween_property(
		...
		original_scale,
		0.2
	).set_delay(0.3).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_CUBIC)
	
	unlock_tween.finished.connect(_finish_unlock_animation)


func _finish_unlock_animation():
	_unlock_sequence(true)


func _unlock_sequence(play_anim: bool) -> void:
	unlocked = true
	...
	if play_anim:
		var items_to_spawn: int = count_of_resources_spawned_on_first_unlock
		while items_to_spawn > 0:
			island_resource_manager.spawn_random_object()
			items_to_spawn -= 1
			await get_tree().create_timer(0.3).timeout
		await get_tree().create_timer(0.5).timeout
		(QuestsManager.npcs[NPC.NAME.Moar] as MOAR).island_finished_unlocking.emit()
#endregion
...
func update_single_tile_data(tile_data: Dictionary):
	...
	if not tile_states.has(tile_position):
		...
		water_layer.set_cell(tile_position, 2, Vector2i(0, 0), 1)

	tile_states[tile_position].update_single_tile_data(tile_data)
...
func _sync_tile_data(tile_data: Dictionary):
	...
	for tile in tile_data:
		...
		if tile_states[tile].tile_state == TileManager.TileState.Bridge:
			water_layer.set_cell(tile, 2, Vector2i(0, 0), 1)
...
func can_tile_be_watered(interacted_tile: Vector2i) -> bool:
	...
	return unlocked and tile and tile.can_tile_be_watered()

func can_tile_be_tilled(interacted_tile: Vector2i) -> bool:
	var tile = get_island_tile_from_vector(interacted_tile)
	return unlocked and tile and tile.can_tile_be_tilled()

func can_tile_be_fertilized(interacted_tile: Vector2i):
	var tile = get_island_tile_from_vector(interacted_tile)
	return unlocked and tile and tile.can_tile_be_fertilized()
	
func can_tile_be_planted(interacted_tile: Vector2i, plant: Item) -> bool:
	var tile = get_island_tile_from_vector(interacted_tile)
	return unlocked and tile and tile.can_tile_be_planted(plant)

func can_tile_be_planted_with_sapling(interacted_tile: Vector2i, sapling: Item) -> bool:
	var tile = get_island_tile_from_vector(interacted_tile)
	return unlocked and tile and tile.can_tile_be_planted_with_sapling(sapling)

func can_tile_be_harvested(interacted_tile: Vector2i) -> bool:
	var tile = get_island_tile_from_vector(interacted_tile)
	return unlocked and tile and tile.can_tile_be_harvested()
	
func can_tile_be_placed_on(interacted_tile: Vector2i, ignore_player: bool = false) -> bool:
	var tile = get_island_tile_from_vector(interacted_tile)
	return unlocked and tile and tile.can_tile_be_placed_on(ignore_player)
...
func can_tile_be_chopped_or_mined(interacted_tile: Vector2i, item: Item) -> bool:
	var tile = get_island_tile_from_vector(interacted_tile)
	return unlocked and tile and tile.can_tile_be_chopped_or_mined(item)

func can_tile_be_non_object_placed(interacted_tile: Vector2i, item: Item) -> bool:
	var tile = get_island_tile_from_vector(interacted_tile)
	return unlocked and tile and tile.can_tile_be_non_object_placed_on(item)
#endregion
...
func load(save_data):
	...
	for tile in save_data["tile_states"]:
		...
		if loaded_tile.tile_state == TileManager.TileState.Bridge:
			water_layer.set_cell(loaded_tile.tile_position, 2, Vector2i(0, 0), 1)
	
	#update all the visuals
	update_all_tile_visuals()
	
	islands_layout = $"/root/Game/IslandsLayout"
	if not islands_layout.finished_initial_load:
		islands_layout.finished_initial_load_signal.connect(_unlock)
	else:
		_unlock()
...
func add_active_tile_to_active_array(tile):
	...
	# Sync to server if this is a client
	if not is_multiplayer_authority():
		sync_active_tile_to_server.rpc_id(1, tile.tile_position, true)
...
func remove_active_tile_from_active_array(tile):
	...
	# Sync to server if this is a client
	if not is_multiplayer_authority():
		sync_active_tile_to_server.rpc_id(1, tile.tile_position, false)


@rpc("any_peer", "call_local", "reliable")
func sync_active_tile_to_server(tile_position: Vector2i, add_to_array: bool):
	if not is_multiplayer_authority():
		return
	...
	var tile = tile_states[tile_position]
	if add_to_array:
		add_active_tile_to_active_array(tile)
	else:
