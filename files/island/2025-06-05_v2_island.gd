...
func _ready() -> void:
	...
	if not is_multiplayer_authority():
		#person who just spawned (client) will request to sync island tiless
		_sync_tile_data_request.rpc_id(1)
		return
...
func _sync_tile_data_request():
	...
	if sender_id != 0:
		_sync_tile_data.rpc_id(sender_id, copy_tile_states_dict())
	else:
		_sync_tile_data(copy_tile_states_dict())
