...
func _ready() -> void:
	...
	for island in get_parent().get_children():
...
func can_do_tile_action(world_position: Vector2, tile_action: TileActions, item_id: StringName) -> bool:
	...
	match tile_action:
		...
		TileActions.Plant:
			return this_island.can_tile_be_planted(relative_tile_vector)
...
func do_tile_action(world_position: Vector2, tile_action: TileActions) -> bool:
	...
	match tile_action:
		...
		TileActions.Fish:
			fishing_tile_changed.emit(world_position)
			return not island_tile_states[relative_island_vector].get_island_tile_from_vector(relative_tile_vector)
		_:
			ErrorHandler.throw_error(107)
	return false
...
func do_tile_action_with_id(world_position: Vector2, tile_action: TileActions, id: StringName) -> bool:
	...
	match tile_action:
		TileManager.TileActions.Chop:
			return tile.use_axe_or_pickaxe_on_tile(item)
		TileManager.TileActions.Mine:
			return tile.use_axe_or_pickaxe_on_tile(item)
...
func assign_object_to_tile(world_position: Vector2, object: Node, targetable: bool = true, ignore_player: bool = false) -> bool:
	var relative_island_vector := get_relative_island_vector(world_position)
	var relative_tile_vector := get_relative_tile_vector(relative_island_vector, world_position)
	...
	return tile.place_object_on_tile(object.get_path(), targetable, true)
...
enum TileActions { Till, Water, Plant, Plant_Sapling, Fish, Harvest, Fertilize, Bridge, Chop, Mine, MachineTill}
