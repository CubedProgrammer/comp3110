...
func can_do_tile_action(world_position: Vector2, tile_action: TileActions, item_id: StringName) -> bool:
	...
	match tile_action:
		...
		TileActions.Plant_Sapling:
			return (
				(ItemDb.get_item(item_id) as SaplingItem).allowed_biomes.has(this_island.biome) and
				can_tile_be_placed_on(world_position, true) and
				not is_tile_bridge(world_position)
			)
		TileActions.Fish:
