...
func load_data():
	...
	while save_file.get_position() < save_file.get_length():
		...
		if node_data["instantiate"]:
			new_object = load(node_data["scene"]).instantiate()
...
func _notification(what):
	if what == NOTIFICATION_WM_CLOSE_REQUEST:
		...
		world.get_player().save.rpc_id(
			1,
			PlayerNetInfo.my_player_info.steam_id,
			world.get_player().player_data.to_dict(),
			Player.SaveType.ExitGame
		)
