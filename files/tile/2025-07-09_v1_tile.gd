...
func can_tile_be_planted(plant: Item) -> bool:
	return (
		plant is PlantableItem and
		plant_on_tile == null and
		(tile_state == TileManager.TileState.Tilled or tile_state == TileManager.TileState.Watered) and
		(plant as PlantableItem).allowed_biomes.has(island.biome)
	)
...
func plant_tile(seed: Item) -> bool:
	if not can_tile_be_planted(seed) or not seed is PlantableItem:
		return false
