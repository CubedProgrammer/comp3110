...
var non_object_on_tile: StringName
var non_object_on_tile_health: int
...
func update_non_objects_on_tile() -> void:
	if not non_object_on_tile:
		island.walls_layer.set_cells_terrain_connect(\
			[tile_position],
			0,
			-1
		)
		island.floors_layer.set_cells_terrain_connect(\
			[tile_position],
			0,
			-1
		)
	...
	var item_on_tile: PlacableItem = ItemDb.get_item(non_object_on_tile)
	var layer_to_modify: TileMapLayer = island.walls_layer if item_on_tile.placable_type == PlacableItem.Category.wall else island.floors_layer
	layer_to_modify.set_cells_terrain_connect(
		[tile_position],
		0,
		item_on_tile.tilseset_index,
	)
...
func can_tile_be_placed_on(ignore_player: bool = false) -> bool:
	...
	if non_object_on_tile and (ItemDb.get_item(non_object_on_tile) as PlacableItem).solid:
		return false
...
func can_tile_be_chopped_or_mined(item: Item) -> bool:
	...
	if hittable_object and can_hit_tile and not hittable_object.is_empty() and island.has_node(hittable_object):	
		var object = island.get_node(hittable_object)
		if (object and object is HittableObject and (object as HittableObject).can_hit(item)):
			return true
	
	if non_object_on_tile and (ItemDb.get_item(non_object_on_tile) as PlacableItem).tools_to_break.has((item as UsableItem).type):
		return true
	
	return false


func can_tile_be_non_object_placed_on(item: Item) -> bool:
	var cell = island.data_layer.get_cell_tile_data(tile_position)
	if cell and cell.get_custom_data("un-interactable"):
		return false
	
	if item is not PlacableItem:
		return false
	
	var placable_item: PlacableItem = item as PlacableItem
	var ignore_player: bool = placable_item.placable_type == PlacableItem.Category.floor
	...
	if placable_item.solid and hittable_object and not hittable_object.is_empty() and island.has_node(hittable_object):
		return false

	return not player_on_tile and hittable_object == NodePath("") and not plant_on_tile and not non_object_on_tile
#endregion
...
func place_non_object_on_tile(placable: PlacableItem) -> bool:
	if not can_tile_be_non_object_placed_on(placable):
		return false
	...
	non_object_on_tile = placable.id
	non_object_on_tile_health = placable.hits_to_break
	island.update_single_tile_data.rpc(to_dict())
...
func use_axe_or_pickaxe_on_tile(item: Item) -> bool:
	...
	elif non_object_on_tile:
		non_object_on_tile_health -= 1
		if non_object_on_tile_health <= 0:
			# drop object
			non_object_on_tile = ""
		island.update_single_tile_data.rpc(to_dict())
...
func to_dict() -> Dictionary:
	return {
		...
		"non_object_on_tile": non_object_on_tile,
		"non_object_on_tile_health": non_object_on_tile_health
	}
...
func override_tile(tile_save_data: Dictionary) -> Tile:
	...
	non_object_on_tile = tile_save_data.non_object_on_tile
	non_object_on_tile_health = tile_save_data.non_object_on_tile_health
	
	if not plant_on_tile:
		island.plant_harvested.emit(tile_position)
