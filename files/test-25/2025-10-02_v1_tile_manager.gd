...
func can_do_tile_action(world_position: Vector2, tile_action: TileActions, item_id: StringName) -> bool:
	...
	match tile_action:
		...
		TileActions.Plant_Sapling:
			return this_island.can_tile_be_planted_with_sapling(relative_tile_vector, ItemDb.get_item(item_id))
		TileActions.Fish:
			return not this_island.get_island_tile_from_vector(relative_tile_vector) and this_island.unlocked
		...
		TileActions.Harvest:
			return this_island.can_tile_be_harvested(relative_tile_vector)
		TileActions.Place:
			return this_island.can_tile_be_non_object_placed(relative_tile_vector, ItemDb.get_item(item_id))
		_:
...
func do_tile_action_with_id(world_position: Vector2, tile_action: TileActions, id: StringName) -> bool:
	...
	match tile_action:
		...
		TileManager.TileActions.Place:
			return tile.place_non_object_on_tile(item)
		_:
			return false
...
func get_tile_at_global_tile_position(global_tile_position: Vector2i) -> Tile:
	var relative_island_vector := get_relative_island_vector(global_tile_position)
	var relative_tile_vector := get_relative_tile_vector(relative_island_vector, global_tile_position)
	if not island_tile_states.has(relative_island_vector):
		return null
		
	return island_tile_states[relative_island_vector].get_island_tile_from_vector(relative_tile_vector)

func get_island_at_global_tile_position(global_tile_position: Vector2i) -> Island:
	var relative_island_vector := get_relative_island_vector(global_tile_position)
	if not island_tile_states.has(relative_island_vector):
		return null
		
	return island_tile_states[relative_island_vector]
...
enum TileActions { Till, Water, Plant, Plant_Sapling, Fish, Harvest, Fertilize, Bridge, Hit, MachineTill, Place}
