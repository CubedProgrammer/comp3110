...
func attempt_spawn_hittable_object(object_scene: PackedScene, position: Vector2, can_spawn_on_bridge: bool = true) -> Node:
	...
	var object_texture: Texture

	if object_instance.has_node("AnimatedSprite2D"):
		object_texture = (object_instance.get_node("AnimatedSprite2D") as AnimatedSprite2D).sprite_frames.get_frame_texture("idle", 0)
	else:
		object_texture = object_instance.texture.texture
	
	for x in range(object_instance.hitbox_size.x):
		for y in range(object_instance.hitbox_size.y):
			if object_instance.not_targetable_tiles.has(Vector2(x, y)):
...
func can_spawn_hittable_object(object_scene: PackedScene, position: Vector2, can_spawn_on_bridge: bool = true, island: Island = null, src: int = -1, asset_index: int = -1) -> Node:
	var object_instance: HittableObject = object_scene.instantiate()
	var tiles_to_occupy: Array[Vector2]
	var object_texture: Texture

	if object_instance.has_node("AnimatedSprite2D"):
		var frames = (object_instance.get_node("AnimatedSprite2D") as AnimatedSprite2D).sprite_frames
		if frames:
			object_texture = frames.get_frame_texture("idle", 0)
		else:
			var atlas_texture = AtlasTexture.new()
			var source = island.plant_layer.tile_set.get_source(src)
			atlas_texture.region = source.get_tile_texture_region(Vector2i(
					src, 
					asset_index
				)
			)
			object_texture = atlas_texture
	else:
		object_texture = object_instance.texture.texture
...
func spawn_harvestable_tree(position: Vector2, island: Island, tilemap_location: Vector2, tree: SaplingItem):
	if not multiplayer.is_server():
		_request_spawn_harvestable_tree.rpc_id(1, position, island.get_path(), tilemap_location, tree.id)
		return

	var harvestable_tree: Sapling = harvestable_tree_scene.instantiate() as Sapling
	harvestable_tree.sapling = tree
