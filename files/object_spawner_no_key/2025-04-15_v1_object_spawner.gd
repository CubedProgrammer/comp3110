...
var chest_1_scene: PackedScene = preload("res://scenes/world/objects/chest_1_scene.tscn")
var furnace_1_scene: PackedScene = preload("res://scenes/world/objects/furnace_1_scene.tscn")

#region Chest
func spawn_chest(position: Vector2, tier: int) -> ChestObject:
	if not multiplayer.is_server():
		_request_chest_spawn.rpc_id(1, position, tier)
		return
...
func spawn_furnace(position: Vector2, tier: int) -> CrafterObject:
	if not multiplayer.is_server():
		_request_furnace_spawn.rpc_id(1, position, tier)
		return
...
@rpc("any_peer", "call_remote", "reliable")
func _request_furnace_spawn(position: Vector2, tier: int):
	if multiplayer.is_server():
		spawn_furnace(position, tier)
#endregion		
