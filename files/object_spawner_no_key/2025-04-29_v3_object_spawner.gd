...
func spawn_object(object_type: ObjectType, tier: int, position: Vector2, tiles_to_occupy:Array = []) -> Node:
	...
	var world_positions_to_occupy: Array[Vector2]
	...
	for tile in tiles_to_occupy:
		...
		world_positions_to_occupy.append(tile * 64)
	
	(new_object as HittableObject).occupied_tiles = world_positions_to_occupy
	return new_object
