...
var furnaces: Array[PackedScene] = [
	preload("res://scenes/world/objects/utility_furniture/furnaces/furnace_1_scene.tscn")
]
...
var harvestable_tree_scene: PackedScene = preload("res://scenes/world/objects/harvestables/harvestable_tree_object.tscn")
#endregion

func _ready() -> void:
	ObjectDataManager.set_object_spawner(self)
...
func attempt_spawn_hittable_object(object_scene: PackedScene, position: Vector2i, can_spawn_on_bridge: bool = true) -> Node:
	...
	var object_id = ObjectDataManager.register_object(object_instance.save())
	
	return ObjectDataManager.active_objects.get(object_id, object_instance)
...
func spawn_harvestable_plant(position: Vector2, island: Island, tilemap_location: Vector2):
	...
	ObjectDataManager.register_object(harvestable_plant.save())
	
func spawn_harvestable_tree(position: Vector2, island: Island, tilemap_location: Vector2, tree: SaplingItem) -> bool:
	if not multiplayer.is_server():
		_request_spawn_harvestable_tree.rpc_id(1, position, island.get_path(), tilemap_location, tree.id)
		return false

	var harvestable_tree: Sapling = can_spawn_hittable_object(harvestable_tree_scene, position)
	...
	var untargetable_tiles: Array[Vector2i] = []
	for x in range(harvestable_tree.hitbox_size.x):
		for y in range(harvestable_tree.hitbox_size.y):
			if harvestable_tree.not_targetable_tiles.has(Vector2i(x, y)):
				untargetable_tiles.append(position + Vector2(x, y) * 64)

	harvestable_tree.sapling = tree as SaplingItem
	harvestable_tree.not_targetable_tiles = untargetable_tiles
	harvestable_tree.position = ((island.layout_position * 18) + Vector2i(tilemap_location) * 64)
	harvestable_tree.current_growth_state = 1
	harvestable_tree.assign_plant(island, tilemap_location)
	harvestable_tree.position.y -= harvestable_tree.texture.get_height()
	return ObjectDataManager.register_object(harvestable_tree.save()).length() > 0
