...
var items_for_machine_array: Array[Item]
...
func update_plant_tile_visual(tile_position: Vector2i):
	...
	match tile_to_update.plant_on_tile_growth_stage:
		...
		5:
			plant_layer.set_cell(tile_position, 3, Vector2i(5, 0))
		-1:
			plant_layer.erase_cell(tile_position)
		_:
			pass
...
func get_harvest_items(tile: Tile , is_for_machine: bool = false):
	...
	for item_dict_entry in tile.plant_on_tile.drop_item_id_array:
		for item_id in item_dict_entry.keys():		
			if should_spawn_item_based_on_percent(item_dict_entry[item_id]):
				object_spawner.spawn_dropable_item(
				TileManager.get_world_position(layout_position , tile.tile_position),
				item_id,
				DropableItem.get_amount_based_on_average(
