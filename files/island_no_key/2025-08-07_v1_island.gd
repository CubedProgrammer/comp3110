...
func _play_unlock_animation() -> void:
	(QuestsManager.npcs[NPC.NAME.Moar] as MOAR).island_unlocking.emit(layout_position)
	
	var original_position = tilemaps.position
	var original_scale = tilemaps.scale
	
	tilemaps.position = original_position + Vector2(0, -300)
	...
	unlock_tween.tween_property(
		...
		original_scale,
		0.6
	).set_delay(0.6).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_CUBIC)
	
	unlock_tween.finished.connect(_unlock_sequence.bind(true))
