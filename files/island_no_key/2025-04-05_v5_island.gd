...
func _process(delta: float) -> void:
	...
	for tile in needs_updating_tile_array:
		if tile.get_tile_timer() <= 0:
			update_tile(tile)
			update_single_tile_data.rpc(tile.get_tile_position(), tile.to_dict())
...
func _sync_tile_data_request():
	...
	_sync_tile_data.rpc_id(
		multiplayer.get_remote_sender_id(),
		copy_tile_states_dict()
	)
...
func can_tile_be_watered(tile_to_till: Vector2i) -> bool:
	return does_island_tile_state_have_tile(tile_to_till) and tile_states[tile_to_till].get_tile_state() == TileManager.TileState.Tilled
...
func can_tile_on_tileset_be_tilled(tile_to_till: Vector2i) -> bool:
	...
	if tile_states[tile_to_till].get_tile_state() != TileManager.TileState.Default:
		return false
