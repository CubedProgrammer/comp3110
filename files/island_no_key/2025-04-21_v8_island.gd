...
func update_highlight_tile_visual(tile_position: Vector2i, highlight: TileManager.TileIndicators) -> void:
	match highlight:
...
func can_tile_be_planted(tile_to_till: Vector2i) -> bool:
	return (
		...
		or (
			...
			and get_island_tile_from_vector(tile_to_till).plant_on_tile == null
		)
	)
