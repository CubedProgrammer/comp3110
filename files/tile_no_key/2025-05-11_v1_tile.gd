...
var hittable_object: NodePath = "":
	set(value):
		if not hittable_object.is_empty() and not value.is_empty():
...
func can_tile_be_placed_on(ignore_player: bool = false) -> bool:
	...
	var player_on_tile = ObjectManager.world.players.any(
	...
	return (not player_on_tile or ignore_player) and hittable_object.is_empty() and not plant_on_tile
...
func plant_tile(seed: PlantableItem) -> bool:
	if not can_tile_be_planted():
		return false
