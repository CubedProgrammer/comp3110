...
func save(uid, player_data) -> void:
	...
	var save_file = FileAccess.open("user://" + str(uid) + ".save", FileAccess.WRITE)
...
func request_load_player_data(uid) -> void:
	...
	if not FileAccess.file_exists("user://" + str(uid) + ".save"):
		return # We don't have a save to load.

	var save_file = FileAccess.open("user://" + str(uid) + ".save", FileAccess.READ)
