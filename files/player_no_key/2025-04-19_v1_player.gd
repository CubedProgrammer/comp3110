...
func _process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("interact") and closest_object:	
		%StateChart.send_event("ui_interact" if not was_in_ui else "idle")
...
func recalculate_closest_object():
	...
	for object in interactable_objects_in_range:
		if global_position.distance_to(object.global_position) < global_position.distance_to(closest_object.global_position) if closest_object else true:
			if closest_object:
				closest_object.highlight = false
				
			closest_object = object
			closest_object.highlight = true
