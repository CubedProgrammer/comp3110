...
func _process(_delta: float) -> void:
	...
	if Input.is_action_just_pressed("interact") and closest_object:
		if closest_object is HarvestablePlantObject:
			%StateChart.send_event("harvest")
		elif closest_object is UIObject:
...
@rpc("authority", "call_local", "reliable")
func set_animation_speed(animated_sprite_speed: float, tool_sprite_speed: float) -> void:
	$AnimatedSprite2D.speed_scale = animated_sprite_speed
	$ToolAnimatedSprite.speed_scale = tool_sprite_speed
